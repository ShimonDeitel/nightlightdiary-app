...
final class Store: ObservableObject {
    ...
    private let fileName = "nightlightdiary_entries.json"
    ...
    private func seed() {
        entries = [
            Entry(date: Calendar.current.date(byAdding: .day, value: -1, to: Date())!, rating: 3, conditions: [], note: ""),\nEntry(date: Calendar.current.date(byAdding: .day, value: -2, to: Date())!, rating: 4, conditions: [], note: ""),\nEntry(date: Calendar.current.date(byAdding: .day, value: -3, to: Date())!, rating: 5, conditions: [], note: "")
        ]
    }
    private func load() {
